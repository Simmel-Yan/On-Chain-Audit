...
func main() {
	...
	// deploy contract VRF
	address, tx, instance, err := VRF.DeployVrf(auth, client)
	if err != nil {
		log.Fatal(err)
	}

	fmt.Printf("contract VRF address: %s, ", address.Hex()) // contract address
	fmt.Printf("tx hash: %s\n", tx.Hash().Hex())            // deploy transaction's tx hash
